...
void AsyncFSWebServer::begin(FS* fs) {
    ...
    if (!MDNS.begin(_config.deviceName.c_str())) {  // Start the mDNS responder
...
void AsyncFSWebServer::send_NTP_configuration_html(AsyncWebServerRequest *request) {
    ...
    handleFileRead("/admin/ntp.html", request);
...
void AsyncFSWebServer::send_wwwauth_configuration_html(AsyncWebServerRequest *request) {
    ...
    handleFileRead("/admin/system.html", request);
...
void AsyncFSWebServer::serverInit() {
    ...
    //load editor
    on("/admin/edit", HTTP_GET, [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        if (!this->handleFileRead("/admin/edit.html", request))
            request->send(404, "text/plain", "FileNotFound");
    });
    //create file
    on("/admin/edit", HTTP_PUT, [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        this->handleFileCreate(request);
    });	//delete file
    on("/admin/edit", HTTP_DELETE, [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
    ...
    on("/admin/edit", HTTP_POST, [](AsyncWebServerRequest *request) { request->send(200, "text/plain", ""); }, [this](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
    ...
    on("/admin/config.html", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
    ...
    on("/admin/general.html", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        this->send_general_configuration_html(request);
    });
    on("/admin/ntp.html", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
    ...
    on("/admin/system.html", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        this->send_wwwauth_configuration_html(request);
    });
    on("/admin/admin.html", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        if (!this->handleFileRead("/admin/admin.html", request))
            request->send(404, "text/plain", "FileNotFound");
    });
    ...
    on("/admin/update", HTTP_GET, [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        if (!this->handleFileRead("/admin/update.html", request))
            request->send(404, "text/plain", "FileNotFound");
    });
    on("/admin/update", HTTP_POST, [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        AsyncWebServerResponse *response = request->beginResponse(200, "text/html", (Update.hasError()) ? "FAIL" : "<META http-equiv=\"refresh\" content=\"15;URL=/admin/update\">Update correct. Restarting...");
    ...
    // GIANLUCA 2018 i risultati di questa funzione mi servono pubblici
    // if (!this->checkAuth(request))
		// 	return request->requestAuthentication();
    ...
    onNotFound([this](AsyncWebServerRequest *request) {
        ...
        // GIANLUCA 2017
        // NOTE abilitando la condizione if in basso ( if (!this->checkAuth...)
        // tutte le richieste a files (html,css,js...) che non rientrano nei casi
        // specifici sopra ( gli handlers on()... )
        // richiederanno le credenziali (se l'opzione è abilitata nelle
        // impostazioni admin -> system settings -> "Use auth" check)
        // Disabilitando la condizione ( if (!this->checkAuth...) i files
        // (che non rientrano nei casi sopra) saranno accessibili
        // pubblicamente di default. A me serve disabilitata dato che ho
        // selezionato sopra specificamente le pagine che richiedono le
        // credenziali di accesso.
        // NOTE ENG: enabling the if statement at the bottom (if (! This-> checkAuth ...)
        // all requests to files (html, css, js ...) that do not fall into the cases
        // specific above (the handlers on () ...) will request the login credentials
        // (if the option is enabled in the
        // admin settings -> system settings -> "Use auth" check)
        // Disabling the condition (if (! This-> checkAuth ...) the files
        // (which do not fit in the above cases) will be accessible
        // publicly by default. I need disabled since I have
        // defined above the pages that require the login credentials.
        // if (!this->checkAuth(request))
        //     return request->requestAuthentication();
